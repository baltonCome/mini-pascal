...
public class TextAreaBorder extends AbstractBorder {
    ...
    private final int characterHeight = 10;
    private final int characterWidth = 5;
    ...
    public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
        ...
        double r = (double) height / (double) this.rowHeight;
        int rows = (int) (r + 0.5);
        ...
        int i = 0;
        if (point.y > 0) {
            i = point.y / this.rowHeight;
        }
        if (d != null) {
            r = (double) d.height / (double) this.rowHeight;
            rows = (int) (r + 0.5);
            rows += i + 1;
        }
        int lenght;
        ...
        g.setColor(oldColor);
        g.setFont(Font.getFont("Consolas"));
    }
